            ...
            try{
                FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("FXMLAnyadirDoctor.fxml"));
                    ...
                    if(!controlador.getCancelar()) {
                        ...
                        if(!d.getName().isEmpty() && !d.getSurname().isEmpty() && !d.getIdentifier().isEmpty() && !d.getTelephon().isEmpty()){
                            lista_doctores_a.add(d);
                            bbdd.getExaminationRooms().add(d.getExaminationRoom());
                            lista_doctores_o= FXCollections.observableArrayList(lista_doctores_a);
                            lista_doctores.setItems(lista_doctores_o);
                            System.out.print("Guardando paciente");
                            bbdd.saveDB();
                        }else{
                        //Alerta campo incorrecto 
                        }
                    }
    ...
    private void addCitaNueva(ActionEvent event) {
        try{
            ...
            if(controlador.guardar()){
                ...
                lista_citas_citas_o = FXCollections.observableList(bbdd.getPatientAppointments(controlador.getPaciente().getIdentifier()));
