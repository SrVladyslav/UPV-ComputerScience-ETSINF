...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        // COMPLETAR
        Arista[] arista = gRM.kruskal();
        double costeOptimo = 0.0;

        // PASO 2. Si kruskal devuelve null, devolver -1.0
        // COMPLETAR
        if(arista == null){return -1.0;}
        ...
        // COMPLETAR
         ...
         adyKruskal = new TablaHash<Municipio,ListaConPI<Municipio>>(arista.length + 1);
        
        for(int i=0;i < arista.length; i++){
            costeOptimo+= arista[i].getPeso();
            Municipio l = verticesAMunicipios.recuperar(arista[i].getOrigen());
            Municipio m = verticesAMunicipios.recuperar(arista[i].getDestino());
            

            ListaConPI<Municipio> I= adyKruskal.recuperar(l);
            if(I == null){
                
                 I = new LEGListaConPI<Municipio>();
            }
            
                
                I.insertar(m);
                adyKruskal.insertar(l,I);
