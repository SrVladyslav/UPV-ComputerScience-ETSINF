...
public class Indexador {
        ...
        try {            
            ...
            while (libro.hasNext()) {
                ...
                for (int i = 0; i < words.length; i++) {
                    ...
                    if (esTermino(clave)) {
                        t1 = System.nanoTime();
                        ListaConPI<Indice> lol = indices.recuperar(clave);
                        if(lol == null){
                            lol = new LEGListaConPI<Indice>();
                            lol.insertar(ind);
                            indices.insertar(clave,lol);

                        }
                        else{lol.insertar(ind);}
    ...
    public ListaConPI<String> indiceDe(String pal) {
        ...
        // COMPLETAR
        ListaConPI<Indice> pol = indices.recuperar(pal.toLowerCase());
        pol.inicio();
        if(pol != null){
        while(!pol.esFin()){
            String s = pol.recuperar().toString();
            res.insertar(s);
            pol.siguiente();
