...
;Examen hecho por Vladyslav Mazurkevych, grupo 3G1
...
(deffacts datos
    ...
    ;tipos de vagon tipo pesoMin pesoMax pesoMaxAdmitido
    (tipo T1 0 15 25);
    (tipo T2 16 23 40)
...
;tren posTren numVagones vagon id tipoVagon ubi enganchado numMaletas pesoVagon maletas maleta nombre cargada entregada nivel
                                                                                              ;si -> 1
                                                                                              ;no -> 0
(deffunction inicio ()
	(reset)
...
(defrule cargarMaleta
    (max-depth ?userDepth)
   	(tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?p 1 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta ?maleta 0 $?c3 nivel ?nivel entregadas $?entr)
   	(maleta ?maleta ?peso ?pos ?)
   	(tipo ?tipoVagon ?min ?max ?pesoPermitido)
	  (test (< ?peso ?max))
	  (test (> ?peso ?min))
    (test (< ?nivel ?userDepth))
    (test (> ?numVagones 0))
    ...
    ;cambio del examen
    (test (<= (+ ?pesoVagon ?peso) ?pesoPermitido))
	=>
	(assert (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?p 1 (+ ?numMaletas 1) (+ ?pesoVagon ?peso) $?vt1 maletas $?vt2 maleta ?maleta 1 $?c3 nivel (+ ?nivel 1) entregadas $?entr))
  (bind ?*nodosgen* (+ ?*nodosgen* 1))
)

(defrule descargarMaleta
   (max-depth ?userDepth)
   (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?p 1 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta ?maleta 1 $?c3 nivel ?nivel entregadas $?entr)
   (maleta ?maleta ?peso ? ?pos)
   (test (> ?numMaletas 0))
   (test (< ?nivel ?userDepth))
   (test (eq (member$ ?maleta $?entr) FALSE))
	...
	=>
	(assert (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?p 1 (- ?numMaletas 1) (- ?pesoVagon ?peso) $?vt1 maletas $?vt2 maleta ?maleta 0 $?c3 nivel (+ ?nivel 1) entregadas $?entr ?maleta))
  (bind ?*nodosgen* (+ ?*nodosgen* 1))
)
...
(defrule engancharVagon
   (max-depth ?userDepth)
   (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?pos 0 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta $?c3 nivel ?nivel entregadas $?entr)
	 (test (< ?nivel ?userDepth))
   ...
   (test (= 0 ?numMaletas));OJO CONDICION DEL ENUNCIADO PERO PUEDE SER QUE PETE POR ESO
	=>
	(assert (tren ?pos (+ ?numVagones 1) $?vt vagon ?vagon ?tipoVagon ?pos 1 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta $?c3 nivel (+ ?nivel 1) entregadas $?entr))
	(bind ?*nodosgen* (+ ?*nodosgen* 1))
)

(defrule desengancharVagon
   (max-depth ?userDepth)
   (tren ?pos ?numVagones $?vt vagon ?vagon ?tipoVagon ?p 1 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta $?c3 nivel ?nivel entregadas $?entr)
   (test (> ?numVagones 0))
    (test (= ?numMaletas 0))
    (test (< ?nivel ?userDepth))
	=>
	(assert (tren ?pos (- ?numVagones 1) $?vt vagon ?vagon ?tipoVagon ?pos 0 ?numMaletas ?pesoVagon $?vt1 maletas $?vt2 maleta $?c3 nivel (+ ?nivel 1) entregadas $?entr))
  (bind ?*nodosgen* (+ ?*nodosgen* 1))
)
